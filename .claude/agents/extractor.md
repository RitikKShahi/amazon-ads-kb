...
When fetching a source, evaluate its type. For standard or static pages, use `tavily_extract` and save the exact Markdown it returns. For Single Page Applications (SPAs) or heavily JS-rendered pages, use `mcp__playwright__browser_navigate` followed by `mcp__playwright__browser_run_code_unsafe` to evaluate `document.body.innerText` and save that text. **CRITICAL**: Immediately save the EXACT output (Markdown or innerText) to a temporary file in `fixtures/` (e.g., `fixtures/<slug>.txt`). Do NOT save your JSON claims to this file. Do NOT add any headers or formatting. Then, run `node scripts/hash_source.js check <url> <path>` via Bash. **If it outputs UNCHANGED, stop immediately and do not extract any claims.**
